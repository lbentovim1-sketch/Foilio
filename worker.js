...
export default {
  async fetch(request, env = {}) {
    ...
    if (url.pathname === "/vault" || url.pathname === "/vault/") {
      return new Response(renderVaultGallery(env), { headers: { "Content-Type": "text/html; charset=utf-8" } });
    }
    if (url.pathname === "/vault/admin" || url.pathname === "/vault/admin/") {
      return new Response(renderVaultAdmin(env), { headers: { "Content-Type": "text/html; charset=utf-8" } });
    }
    if (url.pathname === "/vault/api/cards" && request.method === "GET") {
      if (request.method === "OPTIONS") return new Response(null, { headers: CORS });
      const sbUrl = envValue(env, "SUPABASE_URL");
      const sbKey = envValue(env, "SUPABASE_ANON_KEY") || envValue(env, "SUPABASE_PUBLISHABLE_KEY");
      if (!sbUrl || !sbKey) return jsonResponse({ error: "Supabase not configured." }, 500);
      try {
        const r = await fetch(
        ...
        return jsonResponse({ error: "Could not load cards." }, 502);
      }
    }
    if (url.pathname === "/vault/api/cards" && request.method === "OPTIONS") {
      return new Response(null, { headers: CORS });
    }
    if (url.pathname === "/vault/api/cards" && request.method === "POST") {
      const adminPass = envValue(env, "VAULT_ADMIN_PASS");
      const given = request.headers.get("x-vault-pass") || "";
      if (adminPass && given !== adminPass) return jsonResponse({ error: "Unauthorized." }, 401);
      const sbUrl = envValue(env, "SUPABASE_URL");
      const sbKey = envValue(env, "SUPABASE_SERVICE_KEY") || envValue(env, "SUPABASE_ANON_KEY") || envValue(env, "SUPABASE_PUBLISHABLE_KEY");
      if (!sbUrl || !sbKey) return jsonResponse({ error: "Supabase not configured." }, 500);
      try {
        ...
        return jsonResponse({ error: "Could not save card." }, 502);
      }
    }
    if (url.pathname.startsWith("/vault/api/cards/") && (request.method === "PUT" || request.method === "PATCH")) {
      const adminPass = envValue(env, "VAULT_ADMIN_PASS");
      const given = request.headers.get("x-vault-pass") || "";
      if (adminPass && given !== adminPass) return jsonResponse({ error: "Unauthorized." }, 401);
      const cardId = url.pathname.replace("/vault/api/cards/", "");
      const sbUrl = envValue(env, "SUPABASE_URL");
      const sbKey = envValue(env, "SUPABASE_SERVICE_KEY") || envValue(env, "SUPABASE_ANON_KEY") || envValue(env, "SUPABASE_PUBLISHABLE_KEY");
      if (!sbUrl || !sbKey) return jsonResponse({ error: "Supabase not configured." }, 500);
      try {
        const body = await request.json();
        const r = await fetch(sbUrl + "/rest/v1/vault_cards?id=eq." + encodeURIComponent(cardId), {
          method: "PATCH",
          headers: {
            "apikey": sbKey, "Authorization": "Bearer " + sbKey,
            "Content-Type": "application/json", "Prefer": "return=representation",
        ...
        return jsonResponse({ error: "Could not update card." }, 502);
      }
    }
    if (url.pathname.startsWith("/vault/api/cards/") && request.method === "DELETE") {
      const adminPass = envValue(env, "VAULT_ADMIN_PASS");
      const given = request.headers.get("x-vault-pass") || "";
      if (adminPass && given !== adminPass) return jsonResponse({ error: "Unauthorized." }, 401);
      const cardId = url.pathname.replace("/vault/api/cards/", "");
      ...
      try {
        ...
        return jsonResponse({ error: "Could not delete card." }, 502);
      }
    }
    if (url.pathname === "/vault/api/upload" && request.method === "POST") {
      const adminPass = envValue(env, "VAULT_ADMIN_PASS");
      const given = request.headers.get("x-vault-pass") || "";
      if (adminPass && given !== adminPass) return jsonResponse({ error: "Unauthorized." }, 401);
      const sbUrl = envValue(env, "SUPABASE_URL");
      ...
      try {
        ...
        if (!uploadRes.ok) {
          ...
          return jsonResponse({ error: "Upload failed: " + errText }, 502);
        }
        const publicUrl = sbUrl + "/storage/v1/object/public/vault-images/" + storagePath;
        return jsonResponse({ url: publicUrl }, 200);
      } catch (e) {
        return jsonResponse({ error: "Upload error: " + (e && e.message ? e.message : String(e)) }, 502);
      }
    }
    if (url.pathname === "/vault/api/upload" && request.method === "OPTIONS") {
      return new Response(null, { headers: CORS });
    }
    // ── end LFGMVault routes ───────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  LFGMVault — Public Gallery
// ─────────────────────────────────────────────────────────────────────────────
function renderVaultGallery(env) {
  const igHandle = "lfgmvault";
  const contactEmail = ""; // set your shared gmail here when ready
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
<meta name="description" content="LFGMVault is a group of collectors sharing a vault of premium trading cards. Browse our collection and reach out if you're interested."/>
...
<style>
  :root{
    ...
    --gold:#f5b544;--gold2:#ffcf6a;--indigo:#7c6cff;--violet:#a78bfa;
    --up:#2bd67a;--pill-bg:rgba(245,181,68,.12);--pill-border:rgba(245,181,68,.3);
  }
  *{box-sizing:border-box;margin:0;padding:0}
  html{scroll-behavior:smooth}
  body{background:var(--bg);color:var(--text);font-family:'Manrope',system-ui,sans-serif;line-height:1.6;min-height:100vh;overflow-x:hidden}
  ::-webkit-scrollbar{width:6px}::-webkit-scrollbar-thumb{background:var(--border2);border-radius:4px}
  a{color:var(--gold);text-decoration:none}
  img{display:block}

  /* ── Topbar ── */
  #topbar{position:sticky;top:0;z-index:50;background:rgba(7,8,15,.85);backdrop-filter:blur(16px);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;padding:0 28px;height:62px}
  .tbar-logo{font-family:'Space Grotesk',sans-serif;font-size:21px;font-weight:800;letter-spacing:-0.5px;background:linear-gradient(90deg,var(--gold),var(--gold2) 45%,#fff 70%,var(--gold2));background-clip:text;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-size:200% auto;animation:shimmer 5s linear infinite}
  @keyframes shimmer{to{background-position:200% center}}
  .tbar-links{display:flex;align-items:center;gap:12px}
  .tbar-btn{display:flex;align-items:center;gap:7px;padding:7px 16px;border-radius:100px;font-size:13px;font-weight:700;border:none;cursor:pointer;text-decoration:none;transition:all .2s}
  .tbar-btn.ig{background:linear-gradient(135deg,#f09433,#e6683c 25%,#dc2743 50%,#cc2366 75%,#bc1888);color:#fff}
  .tbar-btn.ig:hover{opacity:.88;transform:translateY(-1px)}
  ...
  .hero-glow{position:absolute;inset:0;background:radial-gradient(ellipse 70% 60% at 50% 0%,rgba(124,108,255,.15) 0%,transparent 70%),radial-gradient(ellipse 40% 40% at 80% 90%,rgba(245,181,68,.08) 0%,transparent 60%);pointer-events:none}
  .hero-eyebrow{display:inline-flex;align-items:center;gap:8px;background:var(--pill-bg);border:1px solid var(--pill-border);border-radius:100px;padding:5px 14px;font-size:12px;font-weight:700;color:var(--gold);letter-spacing:.8px;text-transform:uppercase;margin-bottom:24px}
  .hero-title{font-family:'Space Grotesk',sans-serif;font-size:clamp(40px,7vw,82px);font-weight:800;letter-spacing:-2px;line-height:1;margin-bottom:16px}
  ...
  /* ── Stats bar ── */
  #statsBar{display:flex;align-items:center;justify-content:center;gap:32px;padding:20px 28px;border-top:1px solid var(--border);border-bottom:1px solid var(--border);background:var(--surface)}
  .stat-item{text-align:center}
  .stat-item .sv{font-family:'Space Grotesk',sans-serif;font-size:22px;font-weight:800;color:var(--gold)}
  .stat-item .sl{font-size:11px;color:var(--dim);text-transform:uppercase;letter-spacing:.9px;margin-top:2px}
  ...
  /* ── Grid ── */
  #gallery{max-width:1300px;margin:0 auto;padding:24px 28px 80px;display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:20px}
  .card-tile{background:var(--surface);border:1.5px solid var(--border);border-radius:16px;overflow:hidden;cursor:pointer;transition:transform .2s,border-color .2s,box-shadow .2s;position:relative}
  ...
  .card-img-wrap .grade-badge{position:absolute;top:10px;right:10px;padding:4px 10px;border-radius:100px;font-size:11px;font-weight:800;letter-spacing:.5px;font-family:'Space Grotesk',sans-serif}
  .grade-badge.psa10{background:linear-gradient(135deg,#f5b544,#ff6f00);color:#000}
  .grade-badge.psa9{background:#c0c0c0;color:#111}
  .grade-badge.other-grade{background:var(--surface2);border:1px solid var(--border2);color:var(--gold)}
  ...
  .card-for-sale{display:inline-block;margin-top:8px;font-size:10px;font-weight:800;color:var(--up);letter-spacing:.5px;text-transform:uppercase;background:rgba(43,214,122,.12);border:1px solid rgba(43,214,122,.3);border-radius:100px;padding:2px 8px}

  /* ── Empty state ── */
  #emptyState{display:none;text-align:center;padding:80px 28px;color:var(--dim)}
  #emptyState svg{margin:0 auto 20px;opacity:.3}
  #emptyState h3{font-family:'Space Grotesk',sans-serif;font-size:22px;font-weight:700;color:var(--muted);margin-bottom:8px}

  /* ── Modal ── */
  #modal{display:none;position:fixed;inset:0;z-index:100;background:rgba(7,8,15,.85);backdrop-filter:blur(8px);align-items:center;justify-content:center;padding:20px}
  #modal.open{display:flex}
  #modal-inner{background:var(--surface);border:1.5px solid var(--border2);border-radius:20px;max-width:860px;width:100%;max-height:90vh;overflow-y:auto;display:grid;grid-template-columns:1fr 1fr;position:relative}
  ...
  #modal-close{position:absolute;top:14px;right:14px;background:var(--surface2);border:1px solid var(--border2);color:var(--muted);border-radius:50%;width:34px;height:34px;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:color .15s}
  #modal-close:hover{color:var(--text)}
  .m-grade{display:inline-block;padding:5px 14px;border-radius:100px;font-size:13px;font-weight:800;letter-spacing:.5px;font-family:'Space Grotesk',sans-serif}
  .m-title{font-family:'Space Grotesk',sans-serif;font-size:26px;font-weight:800;line-height:1.2}
  .m-set{font-size:14px;color:var(--muted)}
  .m-row{display:flex;flex-direction:column;gap:2px}
  .m-label{font-size:10px;color:var(--dim);text-transform:uppercase;letter-spacing:.9px;font-weight:700}
  ...
  .m-cta{display:flex;align-items:center;justify-content:center;gap:7px;padding:11px 20px;border-radius:100px;font-size:14px;font-weight:700;text-decoration:none;transition:all .2s;border:none;cursor:pointer;width:100%}
  .m-cta.ig{background:linear-gradient(135deg,#f09433,#e6683c 25%,#dc2743 50%,#cc2366 75%,#bc1888);color:#fff}
  .m-cta.ig:hover{opacity:.88}
  ...
  #contact h2{font-family:'Space Grotesk',sans-serif;font-size:32px;font-weight:800;letter-spacing:-1px;margin-bottom:12px}
  #contact p{color:var(--muted);font-size:16px;max-width:480px;margin:0 auto 32px}
  ...
  .spinner{width:28px;height:28px;border:3px solid var(--border2);border-top-color:var(--gold);border-radius:50%;animation:spin .7s linear infinite;flex-shrink:0}
  @keyframes spin{to{transform:rotate(360deg)}}
...
<!-- Topbar -->
<nav id="topbar">
  <div class="tbar-logo">LFGMVault</div>
  <div class="tbar-links">
    <a class="tbar-btn ig" href="https://instagram.com/${igHandle}" target="_blank" rel="noopener">
...
<section id="hero">
  ...
  <p class="hero-sub">Six collectors. One vault. Premium cards acquired together — displayed here and on our Instagram.</p>
  <div class="hero-ctas">
    <a class="hero-cta primary" href="https://instagram.com/${igHandle}" target="_blank" rel="noopener">
...
<!-- Card Modal -->
<div id="modal" role="dialog" aria-modal="true" aria-label="Card details">
  <div id="modal-inner">
    <img id="modal-img" src="" alt="Card image"/>
    <div id="modal-body">
      <button id="modal-close" aria-label="Close">×</button>
      <div id="m-grade-wrap"></div>
      <div class="m-title" id="m-player"></div>
      <div class="m-set" id="m-set"></div>
      <div id="m-details-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:12px 16px;margin-top:4px"></div>
      <div id="m-notes-wrap" style="display:none"><div class="m-notes" id="m-notes"></div></div>
      <div id="m-sale-wrap" style="display:none">
        <div style="font-size:12px;color:var(--up);font-weight:700;letter-spacing:.5px;text-transform:uppercase">Available</div>
        <div style="font-family:'Space Grotesk',sans-serif;font-size:28px;font-weight:800;color:var(--text)" id="m-price"></div>
      </div>
      <div class="m-contact" style="margin-top:12px">
        <p>Interested in this card? Reach out on Instagram or via email.</p>
        <a class="m-cta ig" href="https://instagram.com/${igHandle}" target="_blank" rel="noopener">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
          DM on Instagram
        </a>
...
<section id="contact">
  ...
  <p>We're open to offers on select cards. Hit us up on Instagram or email us to discuss.</p>
  <div class="contact-btns">
    <a class="contact-btn ig" href="https://instagram.com/${igHandle}" target="_blank" rel="noopener">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
      @${igHandle}
    </a>
...
<footer>
  <p>© 2025 LFGMVault · All cards are the shared property of the LFGM collective · <a href="https://instagram.com/${igHandle}" target="_blank">Instagram</a></p>
</footer>
...
(function(){
  ...
  function gradeBadge(card){
    var g = (card.grade||"").toString().toLowerCase();
    var company = (card.grading_company||"").toUpperCase();
    if(!card.grade || card.grade==="Raw") return '<span class="grade-badge raw">RAW</span>';
    var cls = "other-grade";
    if(g==="10" || g.includes("10")) cls = "psa10";
    else if(g==="9" || g.includes("9")) cls = "psa9";
    var label = company ? company+" "+card.grade : card.grade;
    return '<span class="grade-badge '+cls+'">'+escH(label)+'</span>';
  }

  function gradeBadgeLarge(card){
    var g = (card.grade||"").toString().toLowerCase();
    var company = (card.grading_company||"").toUpperCase();
    if(!card.grade || card.grade==="Raw") return '<span class="m-grade raw" style="background:var(--surface2);border:1px solid var(--border2);color:var(--muted)">RAW</span>';
    var cls = "other-grade";
    if(g==="10"||g.includes("10")) cls = "psa10";
    else if(g==="9"||g.includes("9")) cls = "psa9";
    var style = "";
    if(cls==="psa10") style = 'style="background:linear-gradient(135deg,#f5b544,#ff6f00);color:#000"';
    else if(cls==="psa9") style = 'style="background:#c0c0c0;color:#111"';
    else style = 'style="background:var(--surface2);border:1px solid var(--border2);color:var(--gold)"';
    var label = company ? company+" "+card.grade : card.grade;
    return '<span class="m-grade" '+style+'>'+escH(label)+'</span>';
  }
  ...
  function renderTile(card){
    var img = card.image_url ? '<img src="'+escH(card.image_url)+'" alt="'+escH(card.player||card.title||"Card")+'" loading="lazy"/>' : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:var(--dim)"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg></div>';
    var saleBadge = card.is_for_sale ? '<div class="card-for-sale">For Sale</div>' : "";
    var meta = [card.year, card.card_set].filter(Boolean).join(" · ");
    return '<div class="card-tile" data-id="'+escH(card.id)+'" tabindex="0" role="button" aria-label="'+escH(card.player||card.title||"Card")+'">'
      +'<div class="card-img-wrap">'+img+gradeBadge(card)+'</div>'
      +'<div class="card-info"><div class="card-player">'+escH(card.player||card.title||"Unknown")+'</div>'
      +'<div class="card-meta">'+escH(meta||"Trading Card")+'</div>'
      +saleBadge+'</div></div>';
  }

  function filtered(){
    if(activeCat==="all") return allCards;
    return allCards.filter(function(c){ return (c.category||"").toLowerCase()===(activeCat||"").toLowerCase(); });
  }
  ...
  function render(){
    ...
    gallery.innerHTML = cards.map(renderTile).join("");
    gallery.querySelectorAll(".card-tile").forEach(function(el){
      el.addEventListener("click", function(){ openModal(el.getAttribute("data-id")); });
      el.addEventListener("keydown", function(e){ if(e.key==="Enter"||e.key===" ") openModal(el.getAttribute("data-id")); });
    });
  }
  ...
  function openModal(id){
    ...
    document.getElementById("modal-img").style.display = card.image_url ? "" : "none";
    document.getElementById("m-grade-wrap").innerHTML = gradeBadgeLarge(card);
    document.getElementById("m-player").textContent = card.player||card.title||"Unknown";
    document.getElementById("m-set").textContent = [card.year, card.card_set].filter(Boolean).join(" · ");
    var details = [];
    if(card.grading_company) details.push({l:"Graded By",v:card.grading_company});
    if(card.grade) details.push({l:"Grade",v:card.grade});
    if(card.cert_number) details.push({l:"Cert #",v:card.cert_number});
    if(card.category) details.push({l:"Sport",v:card.category});
    document.getElementById("m-details-grid").innerHTML = details.map(function(d){
      return '<div class="m-row"><div class="m-label">'+escH(d.l)+'</div><div class="m-val">'+escH(d.v)+'</div></div>';
    }).join("");
    var nw = document.getElementById("m-notes-wrap");
    if(card.notes){ nw.style.display=""; document.getElementById("m-notes").textContent=card.notes; }
    else { nw.style.display="none"; }
    var sw = document.getElementById("m-sale-wrap");
    if(card.is_for_sale){ sw.style.display=""; document.getElementById("m-price").textContent=formatPrice(card.asking_price)||"Make an Offer"; }
  ...
  fetch("/vault/api/cards").then(function(r){ return r.json(); }).then(function(cards){
    allCards = Array.isArray(cards) ? cards : [];
    document.getElementById("statCards").textContent = allCards.length;
    var forSale = allCards.filter(function(c){ return c.is_for_sale; }).length;
    document.getElementById("statForSale").textContent = forSale||"0";
    render();
  }).catch(function(){
    document.getElementById("gallery").innerHTML='<div style="padding:60px;text-align:center;color:var(--dim);grid-column:1/-1">Could not load cards. Please try again later.</div>';
  });
})();
</script>
</body>
...
// ─────────────────────────────────────────────────────────────────────────────
//  LFGMVault — Admin Panel
// ─────────────────────────────────────────────────────────────────────────────
function renderVaultAdmin(env) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
<link rel="preconnect" href="https://fonts.googleapis.com"/>
<link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;600;700;800&family=Manrope:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet"/>
<style>
  :root{
    --bg:#07080f;--surface:#0d0f1c;--surface2:#12152a;--border:#1e2240;--border2:#2a2f52;
    --text:#f0f2ff;--muted:#8b91b8;--dim:#4a4f72;
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .logo span{color:var(--muted);font-weight:500;font-size:13px;margin-left:8px}
  .topbar-links{display:flex;gap:12px;align-items:center}
  .link-btn{padding:6px 14px;border-radius:8px;font-size:13px;font-weight:600;border:1px solid var(--border2);background:transparent;color:var(--muted);cursor:pointer;text-decoration:none;transition:all .15s;font-family:'Manrope',sans-serif}
  .link-btn:hover{color:var(--text);border-color:var(--border2)}
  .link-btn.primary{background:var(--indigo);border-color:var(--indigo);color:#fff}
  .link-btn.primary:hover{opacity:.88}
  ...
  #authBox{background:var(--surface);border:1.5px solid var(--border2);border-radius:20px;padding:40px;max-width:380px;width:100%;text-align:center}
  ...
  #authBtn{width:100%;padding:13px;border-radius:10px;background:var(--gold);border:none;color:#000;font-size:15px;font-weight:800;cursor:pointer;font-family:'Manrope',sans-serif;transition:opacity .15s}
  #authBtn:hover{opacity:.88}
  #authErr{color:var(--down);font-size:13px;margin-top:8px;min-height:18px}

  /* ── Layout ── */
  #app{display:none;max-width:1100px;margin:0 auto;padding:32px 24px 80px;gap:32px;display:none;grid-template-columns:420px 1fr}
  #app.visible{display:grid}

  /* ── Upload panel ── */
  .panel{background:var(--surface);border:1.5px solid var(--border);border-radius:16px;padding:24px}
  .panel-title{font-family:'Space Grotesk',sans-serif;font-size:18px;font-weight:700;margin-bottom:20px;display:flex;align-items:center;gap:8px}
  ...
  /* ── Image upload zone ── */
  #dropZone{border:2px dashed var(--border2);border-radius:12px;padding:28px;text-align:center;cursor:pointer;transition:border-color .2s,background .2s;position:relative;margin-top:14px}
  #dropZone.drag{border-color:var(--gold);background:rgba(245,181,68,.05)}
  #dropZone input{position:absolute;inset:0;opacity:0;cursor:pointer;width:100%;height:100%}
  #dropZone svg{color:var(--dim);margin:0 auto 8px}
  ...
  #previewWrap img{width:100%;max-height:240px;object-fit:contain;border-radius:10px;border:1px solid var(--border2)}
  #previewWrap .clear{margin-top:8px;font-size:12px;color:var(--down);cursor:pointer}
  ...
  .slider{position:absolute;inset:0;background:var(--border2);border-radius:24px;cursor:pointer;transition:.2s}
  ...
  /* ── Submit btn ── */
  #submitBtn{width:100%;margin-top:20px;padding:13px;border-radius:10px;background:var(--gold);border:none;color:#000;font-size:15px;font-weight:800;cursor:pointer;font-family:'Manrope',sans-serif;transition:opacity .15s;display:flex;align-items:center;justify-content:center;gap:8px}
  #submitBtn:disabled{opacity:.5;cursor:default}
  #submitMsg{margin-top:10px;font-size:13px;text-align:center;min-height:18px}
  #submitMsg.ok{color:var(--up)}
  #submitMsg.err{color:var(--down)}

  /* ── Card list ── */
  #cardList{display:flex;flex-direction:column;gap:10px}
  .card-row{background:var(--surface2);border:1px solid var(--border);border-radius:12px;display:flex;align-items:center;gap:12px;padding:10px 14px;transition:border-color .15s}
  .card-row:hover{border-color:var(--border2)}
  .card-row-img{width:40px;height:56px;border-radius:6px;object-fit:cover;background:var(--bg);flex-shrink:0}
  .card-row-info{flex:1;min-width:0}
  .card-row-name{font-size:14px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .card-row-meta{font-size:11px;color:var(--muted)}
  .card-row-actions{display:flex;gap:6px;flex-shrink:0}
  .action-btn{padding:5px 10px;border-radius:7px;font-size:12px;font-weight:700;border:none;cursor:pointer;font-family:'Manrope',sans-serif;transition:opacity .15s}
  .action-btn:hover{opacity:.8}
  .action-btn.del{background:rgba(255,93,108,.15);color:var(--down);border:1px solid rgba(255,93,108,.3)}
  .action-btn.vis{background:var(--surface);border:1px solid var(--border2);color:var(--muted)}
  .action-btn.vis.on{background:rgba(43,214,122,.1);border-color:rgba(43,214,122,.3);color:var(--up)}
  .no-cards{text-align:center;padding:48px 24px;color:var(--dim)}
  .no-cards svg{margin:0 auto 12px;opacity:.4}
...
<div id="topbar">
  ...
  <div class="topbar-links">
    <a class="link-btn" href="/vault" target="_blank">View Gallery →</a>
  </div>
</div>
...
<div id="authScreen">
  <div id="authBox">
    <h2>🔐 Vault Admin</h2>
    <p>Enter your admin password to manage the vault.</p>
  ...
  <div>
    <div class="panel">
      ...
      <div id="previewWrap">
        ...
        <div class="clear" id="clearPreview">✕ Remove image</div>
      </div>
      ...
      <div class="field-row">
        <div>
          ...
          <input type="text" id="fYear" placeholder="e.g. 2023"/>
        </div>
        <div>
          <label for="fSet">Set / Product</label>
          <input type="text" id="fSet" placeholder="e.g. Prizm"/>
        </div>
      </div>
      ...
      <div class="field-row">
        ...
        <div>
          ...
          <input type="text" id="fGrade" placeholder="e.g. 10"/>
        </div>
      </div>

      <!-- Cert # -->
      <label for="fCert">Cert / Population # (optional)</label>
      <input type="text" id="fCert" placeholder="e.g. 12345678"/>

      <!-- Notes -->
      <label for="fNotes">Notes (optional)</label>
      <textarea id="fNotes" placeholder="e.g. Short print, /25, rookie card…"></textarea>

      <!-- Toggles -->
      <div style="margin-top:14px">
        <div class="toggle-row">
          <div><div class="toggle-label">Visible on Gallery</div><div class="toggle-sub">Show this card publicly</div></div>
          <label class="switch"><input type="checkbox" id="fVisible" checked/><span class="slider"></span></label>
        </div>
        <div class="toggle-row">
          <div><div class="toggle-label">Mark as For Sale</div><div class="toggle-sub">Show "For Sale" badge on card</div></div>
      ...
      <div id="priceRow" style="display:none;margin-top:14px">
        ...
        <input type="number" id="fPrice" placeholder="e.g. 500" min="0" step="1"/>
      </div>
...
(function(){
  ...
  function tryAuth(){
    ...
    fetch("/vault/api/cards",{ headers:{"x-vault-pass":p} }).then(function(r){
      if(r.ok){
        adminPass = p;
        ...
        document.getElementById("authErr").textContent="Wrong password. Try again.";
      }
    }).catch(function(){
      document.getElementById("authErr").textContent="Could not connect.";
    });
  }
  ...
  document.getElementById("submitBtn").addEventListener("click", async function(){
    ...
    if(!player){ setMsg("err","Player / card title is required."); return; }
    var btn = document.getElementById("submitBtn");
    btn.disabled=true; btn.innerHTML='<div style="width:16px;height:16px;border:2.5px solid rgba(0,0,0,.3);border-top-color:#000;border-radius:50%;animation:spin .6s linear infinite"></div> Saving…';
    setMsg("","");
    try {
      var imageUrl = "";
      if(selectedFile){
        ...
        var up = await fetch("/vault/api/upload",{ method:"POST", headers:{"x-vault-pass":adminPass}, body:fd });
      ...
      var r = await fetch("/vault/api/cards",{ method:"POST", headers:{"Content-Type":"application/json","x-vault-pass":adminPass}, body:JSON.stringify(payload) });
      if(!r.ok){ var ej=await r.json(); throw new Error(ej.error||"Save failed"); }
      setMsg("ok","✓ Card added to vault!");
      resetForm();
      loadCards();
    } catch(e){
      setMsg("err","Error: "+(e.message||"Unknown error"));
    }
    btn.disabled=false; btn.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12l7-7 7 7"/></svg> Add to Vault';
  });
  ...
  function resetForm(){
    ...
    document.getElementById("fCert").value="";
    document.getElementById("fNotes").value="";
    document.getElementById("fVisible").checked=true;
    document.getElementById("fForSale").checked=false;
  ...
  function loadCards(){
    ...
    fetch("/vault/api/cards",{headers:{"x-vault-pass":adminPass}}).then(function(r){ return r.json(); }).then(function(cards){
      allCards = Array.isArray(cards) ? cards : [];
      document.getElementById("listStatus").textContent = allCards.length+" card"+(allCards.length!==1?"s":"")+" in vault";
      renderList();
    }).catch(function(){
      document.getElementById("listStatus").textContent="Could not load cards.";
    });
  }
  ...
  function renderList(){
    ...
    if(!allCards.length){
      el.innerHTML='<div class="no-cards"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="M3 9h18M9 21V9"/></svg><div>No cards yet. Add your first card!</div></div>';
      return;
    }
    el.innerHTML = allCards.map(function(c){
      ...
      var imgEl = c.image_url ? '<img class="card-row-img" src="'+escH(c.image_url)+'" alt=""/>' : '<div class="card-row-img" style="display:flex;align-items:center;justify-content:center;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg></div>';
      return '<div class="card-row" data-id="'+escH(c.id)+'">'
        +imgEl
        +'<div class="card-row-info"><div class="card-row-name">'+escH(c.player||c.title||"Unknown")+'</div><div class="card-row-meta">'+escH(meta)+'</div></div>'
        +'<div class="card-row-actions">'
        +'<button class="action-btn vis'+(c.is_visible?" on":"")+'">'+( c.is_visible?"Visible":"Hidden")+'</button>'
    ...
    el.querySelectorAll(".card-row").forEach(function(row){
      ...
      row.querySelector(".del").addEventListener("click",function(){
        if(!confirm("Delete this card from the vault?")) return;
        fetch("/vault/api/cards/"+encodeURIComponent(id),{ method:"DELETE", headers:{"x-vault-pass":adminPass} }).then(function(r){
          if(r.ok) loadCards();
